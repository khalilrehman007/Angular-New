...
export class ListpropertyinfoComponent implements OnInit, AfterViewInit {
  ...
  checkLength(type: number) {
    if (type == 1) {
      ...
      if (temp > 200) {
        this.error = "الحد الأقصى لعدد الطوابق المسموح بها هو 200";
        this.showError = true;
        this.SubmitForm.patchValue({
          TotalFloor: "200"
        })
      ...
      if (temp > total) {
        this.error = "لا يمكن أن يكون رقم الطابق أكبر من إجمالي عدد الطوابق";
        this.showError = true;
        this.SubmitForm.patchValue({
